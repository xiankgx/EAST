...
def get_geo_loss(gt_geo, pred_geo):
    d1_gt, d2_gt, d3_gt, d4_gt, angle_gt = torch.split(gt_geo,
                                                       1, 1)
    d1_pred, d2_pred, d3_pred, d4_pred, angle_pred = torch.split(pred_geo,
                                                                 1, 1)

    area_gt = (d1_gt + d2_gt) * (d3_gt + d4_gt)
    area_pred = (d1_pred + d2_pred) * (d3_pred + d4_pred)

    # w_i and h_i in Eqn. 8
...
class Loss(nn.Module):
    def __init__(self, weight_angle: float = 10.0):
        ...
        Args:
            weight_angle (float, optional): Angle loss weight. Defaults to 10.0.
        """

        super(Loss, self).__init__()
        self.weight_angle = weight_angle  # L_theta in Eq. 11
