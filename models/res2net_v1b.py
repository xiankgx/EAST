...
class Res2NetFeat(nn.Module):

    def __init__(self, block, layers, baseWidth=26, scale=4,
                 #  num_classes=1000
                 ):
        self.inplanes = 64
        super(Res2NetFeat, self).__init__()
        self.baseWidth = baseWidth
        self.scale = scale
...
def res2net50_v1b_feature_extractor(pretrained=False, **kwargs):
    """Constructs a Res2Net-50_v1b model.
    Res2Net-50 refers to the Res2Net-50_v1b_26w_4s.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = Res2NetFeat(Bottle2neck, [3, 4, 6, 3], baseWidth=26, scale=4, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['res2net50_v1b_26w_4s'],
                                                 map_location=torch.device("cpu")),
                              strict=False)
...
def res2net101_v1b_feature_extractor(pretrained=False, **kwargs):
    """Constructs a Res2Net-50_v1b_26w_4s model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = Res2NetFeat(Bottle2neck, [3, 4, 23, 3],
                    baseWidth=26, scale=4, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['res2net101_v1b_26w_4s'],
                                                 map_location=torch.device("cpu")),
