"""Adapted from: https://github.com/sanghoon/pytorch_imagenet/blob/master/models/pvanet.py"""
...
class Inception(nn.Module):
    ...
    def add_convs(self, n_kernels, n_chns):
        ...
        for k, n_out in zip(n_kernels, n_chns):
            ...
            # Initialize params
            conv = nn.Conv2d(n_last, n_out, kernel_size=k,
                             bias=False, padding=int(k / 2), stride=stride)
    ...
    def add_poolconv(self, kernel, n_out, type='MAX'):
        ...
        # Pooling
        if type == 'MAX':
            layers.append(nn.MaxPool2d(kernel, padding=int(
                kernel/2), stride=self.in_stride))
        elif type == 'AVE':
            layers.append(nn.AvgPool2d(kernel, padding=int(
                kernel/2), stride=self.in_stride))
...
class PVANetFeat(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv2 = nn.Sequential(
            mCReLU_residual(32, 24, 24, 64, kernelsize=3,
                            preAct=True, lastAct=False, in_stride=1, proj=True),
            mCReLU_residual(64, 24, 24, 64, kernelsize=3,
                            preAct=True, lastAct=False),
            mCReLU_residual(64, 24, 24, 64, kernelsize=3,
                            preAct=True, lastAct=False)
        )
        ...
        self.conv3 = nn.Sequential(
            mCReLU_residual(64, 48, 48, 128, kernelsize=3,
                            preAct=True, lastAct=False, in_stride=2, proj=True),
            mCReLU_residual(128, 48, 48, 128, kernelsize=3,
                            preAct=True, lastAct=False),
            mCReLU_residual(128, 48, 48, 128, kernelsize=3,
                            preAct=True, lastAct=False),
            mCReLU_residual(128, 48, 48, 128, kernelsize=3,
